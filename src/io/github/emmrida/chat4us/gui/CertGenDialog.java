...
/**
 * This is used to generate a new certificate to secure communication between chat server instances,
 * IFrames and messenger apps.
 *
...
public class CertGenDialog extends JDialog {
    ...
    public static String checkStrength(String password) {
        ...
        if (length <= 8)
            return Color.RED.getRGB() + Messages.getString("CertGenDialog.WEAK"); //$NON-NLS-1$
        ...
        if (strengthScore < 3) {
            ...
            return Color.ORANGE.getRGB() + Messages.getString("CertGenDialog.MEDIUM"); //$NON-NLS-1$
        } else if (strengthScore == 4 && length >= 16) {
            return Color.GREEN.getRGB() + Messages.getString("CertGenDialog.STRONG"); //$NON-NLS-1$
        } else {
            return Color.ORANGE.getRGB() + Messages.getString("CertGenDialog.MEDIUM"); //$NON-NLS-1$
        }
    }
	...
	public CertGenDialog(Frame parent) {
		super(parent, Messages.getString("CertGenDialog.DLG_TITLE"), true); //$NON-NLS-1$
		setModalityType(ModalityType.APPLICATION_MODAL);
		//setModal(true);
		//setTitle(Messages.getString("CertGenDialog.DLG_TITLE")); //$NON-NLS-1$
		setResizable(false);
		setBounds(100, 100, 466, 398);
		...
		tabbedPane.addTab(Messages.getString("CertGenDialog.TAB_TITLE_AUTHENTIFICATION"), null, pnlAuth, null); //$NON-NLS-1$

		tfCurPswd = new JPasswordField();
		tfCurPswd.setColumns(10);

		JLabel lblNewLabel_7 = new JLabel(Messages.getString("CertGenDialog.CURRENT_PASSWORD")); //$NON-NLS-1$
		...
		JLabel lblNewLabel_8 = new JLabel(Messages.getString("CertGenDialog.NEW_PASSWORD")); //$NON-NLS-1$
		...
		tfConfPswd.getDocument().addDocumentListener(new DocumentListener() {
			...
			private void checkPassword() {
				...
				if(new String(tfNewPswd.getPassword()).equals(new String(tfConfPswd.getPassword()))) {
					...
					lblConfPswd.setText(Messages.getString("CertGenDialog.LBL_OK")); //$NON-NLS-1$
				} else {
					lblConfPswd.setBackground(Color.RED);
					lblConfPswd.setText(Messages.getString("CertGenDialog.LBL_ERR")); //$NON-NLS-1$
				}
			}
		});
				...
				.addGroup(gl_pnlAuth.createSequentialGroup()
					...
					.addGroup(gl_pnlAuth.createParallelGroup(Alignment.TRAILING)
						...
						.addGroup(gl_pnlAuth.createSequentialGroup()
							...
							.addGroup(gl_pnlAuth.createParallelGroup(Alignment.TRAILING)
								.addComponent(tfCurPswd, GroupLayout.DEFAULT_SIZE, 315, Short.MAX_VALUE)
								.addGroup(gl_pnlAuth.createSequentialGroup()
									.addGroup(gl_pnlAuth.createParallelGroup(Alignment.TRAILING)
										.addComponent(tfConfPswd, GroupLayout.DEFAULT_SIZE, 311, Short.MAX_VALUE)
										.addComponent(tfNewPswd, GroupLayout.DEFAULT_SIZE, 311, Short.MAX_VALUE))
									.addPreferredGap(ComponentPlacement.RELATED)
		...
		cmbDuration.setModel(new DefaultComboBoxModel<>(new String[] {Messages.getString("CertGenDialog.DURATION_ITEM_1MONTH"), Messages.getString("CertGenDialog.DURATION_ITEM_3MONTHS"), Messages.getString("CertGenDialog.DURATION_ITEM_6MONTHS")})); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

		cmbBits = new JComboBox<>();
		cmbBits.setModel(new DefaultComboBoxModel<>(new String[] {Messages.getString("CertGenDialog.BITS_2048"), Messages.getString("CertGenDialog.BITS_4096"), Messages.getString("CertGenDialog.BITS_8192")})); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

		JLabel lblNewLabel_12 = new JLabel(Messages.getString("CertGenDialog.LBL_BITS")); //$NON-NLS-1$

		tfDnsNames = new JTextField();
		tfDnsNames.setToolTipText(Messages.getString("CertGenDialog.TOOLTIP_DNS_IPS_CSV")); //$NON-NLS-1$
		...
		tfIPs.setToolTipText(Messages.getString("CertGenDialog.TOOLTIP_DNS_IPS_CSV")); //$NON-NLS-1$
		tfIPs.setColumns(10);
			...
			gl_pnlInfo.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_pnlInfo.createSequentialGroup()
					...
					.addGroup(gl_pnlInfo.createParallelGroup(Alignment.LEADING)
						...
						.addComponent(cmbBits, Alignment.TRAILING, 0, 326, Short.MAX_VALUE)
						.addComponent(cmbDuration, Alignment.TRAILING, 0, 326, Short.MAX_VALUE)
						.addComponent(tfOrg, GroupLayout.DEFAULT_SIZE, 326, Short.MAX_VALUE)
						.addComponent(tfCity, GroupLayout.DEFAULT_SIZE, 326, Short.MAX_VALUE)
						.addComponent(tfCountry, GroupLayout.DEFAULT_SIZE, 326, Short.MAX_VALUE)
						.addComponent(tfDepart, GroupLayout.DEFAULT_SIZE, 326, Short.MAX_VALUE)
						.addComponent(tfState, GroupLayout.DEFAULT_SIZE, 326, Short.MAX_VALUE)
						.addComponent(tfDomain, GroupLayout.DEFAULT_SIZE, 326, Short.MAX_VALUE)
						.addComponent(tfDnsNames, GroupLayout.DEFAULT_SIZE, 326, Short.MAX_VALUE)
						.addComponent(tfIPs, GroupLayout.DEFAULT_SIZE, 326, Short.MAX_VALUE))
					.addGap(6))
		);
		...
		contentPanel.setLayout(gl_contentPanel);
		{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if(Color.ORANGE.equals(lblNewPswd.getBackground())) {
							if(Messages.getString("CertGenDialog.OK_VALUE").equals(lblConfPswd.getText()) && tfCurPswd.getPassword().length > 0) { //$NON-NLS-1$
								if(!new String(tfNewPswd.getPassword()).equals(new String(tfCurPswd.getPassword()))) { // != current password
									int ret = Helper.showConfirmDialog(CertGenDialog.this, Messages.getString("CertGenDialog.MB_WARNING_CERT_GEN"), Messages.getString("CertGenDialog.MB_WARNING_CERT_GEN_TITLE"), JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE, 1); //$NON-NLS-1$ //$NON-NLS-2$
										...
										new Thread(() -> {
											try (InputStream fos = new java.io.FileInputStream(KEYSTORE_PFX)) {
												...
												SwingUtilities.invokeLater(() -> {
													...
													Helper.logError(ex, Messages.getString("CertGenDialog.ERROR_WRONG_PASSWORD"), true); //$NON-NLS-1$
												});
												return;
											}
											...
											if(generateCertificate()) {
												SwingUtilities.invokeLater(() -> {
													Helper.logInfo(Messages.getString("CertGenDialog.NEW_CERT_GEN_SUCCESS"), true); //$NON-NLS-1$
													cancelled = false;
													setVisible(false);
												});
												return;
						...
						Helper.logInfo(Messages.getString("CertGenDialog.DATA_ERROR"), true); //$NON-NLS-1$
					}
				});
			...
			{
				cancelButton = new JButton(Messages.getString("CertGenDialog.BTN_CANCEL")); //$NON-NLS-1$
